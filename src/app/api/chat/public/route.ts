...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!chatbot) {
      return NextResponse.json({ error: "Chatbot not found or not published" }, { status: 404 });
    }

    const admin = adminClient();
    const limitResult = await checkMessageLimit(admin, chatbot.user_id);
    if (!limitResult.ok) {
    ...
    const { data: chunks } = await supabase.rpc("public_match_chunks_for_chatbot", {
    ...
    await incrementMessageUsage(admin, chatbot.user_id);
